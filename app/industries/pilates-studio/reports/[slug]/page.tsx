...
import { pilatesReports, pilatesReportSlugs } from "@/lib/industries/pilates-reports";
...
export function generateStaticParams() {
  return pilatesReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = pilatesReports[slug];
  if (!report) return { title: "Report Not Found | BizMetricsHQ" };

  return {
    title: `${report.meta.title} — 2026 Pilates Industry Report | BizMetricsHQ`,
    description: report.meta.description,
  };
}

export default async function PilatesBenchmarkReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = pilatesReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = pilatesReportSlugs
    .filter((s) => s !== slug)
    .map((s) => pilatesReports[s].meta);
      ...
      <main>
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/pilates-studio/" className="hover:text-accent">
                Pilates Studio
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <h2 className="font-display text-lg font-semibold text-ink">Related Pilates Reports</h2>
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/pilates-studio/#rankings"
