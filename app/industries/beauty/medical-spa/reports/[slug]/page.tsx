...
import { medicalSpaReports, medicalSpaReportSlugs } from "@/lib/industries/beauty/medical-spa-reports";
...
export function generateStaticParams() {
  return medicalSpaReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = medicalSpaReports[slug];
  if (!report) return { title: "Report Not Found | BizMetricsHQ" };

  return {
    title: `${report.meta.title} | BizMetricsHQ`,
    description: report.meta.description,
  };
}

export default async function MedicalSpaReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = medicalSpaReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = medicalSpaReportSlugs
    .filter((s) => s !== slug)
    .map((s) => medicalSpaReports[s].meta);
        ...
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/beauty/medical-spa/" className="hover:text-accent">
                Medical Spa
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
            ...
            <p className="mt-6 text-xs font-semibold tracking-wider text-accent uppercase">
              Beauty Reports · {meta.readTime} read
            </p>
        ...
        {siblingReports.length > 0 && (
          <section className="border-b border-border/60 bg-surface-muted/50">
            <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
              <h2 className="font-display text-lg font-semibold text-ink">Related Beauty Reports</h2>
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/beauty/medical-spa/#reports"
              ...
              >
                Medical Spa Valuation Calculator
              </Link>
            </div>
          </div>
        </section>
