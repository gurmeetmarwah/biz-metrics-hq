...
import { nailSalonReports, nailSalonReportSlugs } from "@/lib/industries/beauty/nail-salon-reports";
...
export function generateStaticParams() {
  return nailSalonReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = nailSalonReports[slug];
...
export default async function NailSalonReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = nailSalonReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = nailSalonReportSlugs
    .filter((s) => s !== slug)
    .map((s) => nailSalonReports[s].meta);
        ...
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/beauty/nail-salon/" className="hover:text-accent">
                Nail Salon
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/beauty/nail-salon/#reports"
              ...
              >
                Nail Salon Valuation Calculator
              </Link>
            </div>
          </div>
        </section>
