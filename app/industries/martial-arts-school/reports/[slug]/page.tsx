...
import { martialArtsReports, martialArtsReportSlugs } from "@/lib/industries/martial-arts-reports";
...
export function generateStaticParams() {
  return martialArtsReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = martialArtsReports[slug];
  if (!report) return { title: "Report Not Found | BizMetricsHQ" };

  return {
    title: `${report.meta.title} — 2026 Martial Arts Industry Report | BizMetricsHQ`,
    description: report.meta.description,
  };
}

export default async function MartialArtsBenchmarkReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = martialArtsReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = martialArtsReportSlugs
    .filter((s) => s !== slug)
    .map((s) => martialArtsReports[s].meta);
      ...
      <main>
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/martial-arts-school/" className="hover:text-accent">
                Martial Arts School
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <h2 className="font-display text-lg font-semibold text-ink">Related Martial Arts Reports</h2>
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/martial-arts-school/#rankings"
              ...
              <Link
                href="/industries/martial-arts-school/#calculators"
