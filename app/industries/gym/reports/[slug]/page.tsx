...
import { fitnessReports, fitnessReportSlugs } from "@/lib/industries/fitness-reports";
...
export function generateStaticParams() {
  return fitnessReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = fitnessReports[slug];
  if (!report) return { title: "Report Not Found | BizMetricsHQ" };

  return {
    title: `${report.meta.title} — 2026 Fitness Benchmark Report | BizMetricsHQ`,
    description: report.meta.description,
  };
}

export default async function GymBenchmarkReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = fitnessReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = fitnessReportSlugs
    .filter((s) => s !== slug)
    .map((s) => fitnessReports[s].meta);
      ...
      <main>
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/gym/" className="hover:text-accent">
                Gym
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
              </span>
              <span className="text-ink-muted">Benchmark Report</span>
            </nav>
            <p className="mt-6 text-xs font-semibold tracking-wider text-accent uppercase">
              Fitness Benchmark Rankings · {meta.readTime} read
            </p>
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <h2 className="font-display text-lg font-semibold text-ink">Related Benchmark Reports</h2>
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <p className="mt-6 text-sm text-ink-muted">
              Benchmark report figures cross-referenced against: {meta.sources.join(" · ")}.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/gym/#rankings"
