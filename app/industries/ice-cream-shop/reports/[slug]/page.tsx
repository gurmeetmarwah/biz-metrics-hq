...
import { foodReports, foodReportSlugs } from "@/lib/industries/food-reports";
...
export function generateStaticParams() {
  return foodReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = foodReports[slug];
  if (!report) return { title: "Report Not Found | BizMetricsHQ" };

  return {
    title: `${report.meta.title} — 2026 Ice Cream Industry Report | BizMetricsHQ`,
    description: report.meta.description,
  };
}

export default async function IceCreamBenchmarkReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = foodReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = foodReportSlugs
    .filter((s) => s !== slug)
    .map((s) => foodReports[s].meta);
      ...
      <main>
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/ice-cream-shop/" className="hover:text-accent">
                Ice Cream Shop
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
            ...
            <p className="mt-6 text-xs font-semibold tracking-wider text-accent uppercase">
              Food Rankings · {meta.readTime} read
            </p>
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <h2 className="font-display text-lg font-semibold text-ink">Related Ice Cream Reports</h2>
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/ice-cream-shop/#rankings"
              ...
              >
                Ice Cream Valuation Calculator
              </Link>
            </div>
          </div>
        </section>
