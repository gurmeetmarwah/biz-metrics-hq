...
import { dentalReports, dentalReportSlugs } from "@/lib/industries/healthcare/dental-reports";
...
export function generateStaticParams() {
  return dentalReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = dentalReports[slug];
...
export default async function DentalPracticeReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = dentalReports[slug];
  if (!report) notFound();
        ...
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/healthcare/dental-practice/" className="hover:text-accent">
                Dental Practice
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Link
                href="/industries/healthcare/dental-practice/#benchmark"
