...
import { PhysicalTherapyBenchmarkTool } from "@/components/industry/PhysicalTherapyBenchmarkTool";
...
import { revenueDistribution } from "@/lib/industries/physical-therapy-clinic";
import {
  benchmarkThresholds,
  businessScorecard,
  costStructureFlow,
...
export default function PhysicalTherapyClinicHubPage() {
  const faqJsonLd = {
  ...
  const revenueBenchmarkGroups = [
    { title: "Revenue By Clinic Type", cards: revenueByTypeCards },
    { title: "Revenue By Therapist Count", cards: revenueByTherapistCountCards },
    ...
    <>
      ...
      <main>
        <IndustryHubHero
          ...
          currentPageLabel="Physical Therapy Clinic"
          dataVintage={hubMeta.dataVintage}
          sampleSize={hubMeta.sampleSize}
          searchPlaceholder="Search PT metrics..."
          searchLabel="Search PT metrics"
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:items-start">
              <div>
                <h2 className="font-display text-xl font-semibold text-ink sm:text-2xl">
                  Physical Therapy Clinic At A Glance
                </h2>
                <div className="mt-4 h-0.5 w-10 rounded-full bg-accent" aria-hidden="true" />
              ...
              <div id="benchmark">
                <PhysicalTherapyBenchmarkTool thresholds={benchmarkThresholds} />
              </div>
            </div>
          </div>
        </section>
        ...
        <HubSection
          id="explore"
          title="Explore Physical Therapy Economics"
        ...
        <HubSection
          id="calculators"
          title="Physical Therapy Calculators"
          subtitle="Run the numbers on clinic value, patient revenue, EBITDA, break-even, therapist productivity, and startup investment."
        ...
        <HubSection
          id="types"
          title="Physical Therapy Practice Types"
          subtitle="Economics and benchmarks by clinic model — general, sports, orthopedic, pediatric, home health, and multi-location."
        >
        ...
        <HubSection
          ...
          subtitle="Annual revenue benchmarks by clinic type, therapist count, and location."
          className="bg-surface-muted/50"
        >
        ...
        <HubSection
          ...
          subtitle="The patient funnel from referral through evaluation, treatment plan, visits, and collections."
        >
        ...
        <HubSection
          id="referral-economics"
          title="Referral Source Economics"
          subtitle="How referral channels drive patient volume, treatment visits, and revenue per case."
          className="bg-surface-muted/50"
        >
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
              <h3 className="font-display font-semibold text-ink">Referral Funnel</h3>
              <div className="mt-6 flex flex-col items-center gap-2 text-sm">
                {["Physician Referral", "Patient", "Treatment Visits", "Revenue"].map((step, i, arr) => (
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-surface">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border-subtle bg-surface-muted/50">
                    <th className="px-5 py-3.5 text-left font-semibold text-ink">Referral Source</th>
                    <th className="px-5 py-3.5 text-right font-semibold text-ink">Revenue Potential</th>
                  </tr>
                </thead>
                <tbody>
                  {referralEconomicsMetrics.map((row, i) => (
                    <tr
                      key={row.source}
                      className={i < referralEconomicsMetrics.length - 1 ? "border-b border-border-subtle" : ""}
                    >
                      <td className="px-5 py-4 font-medium text-ink-muted">{row.source}</td>
        ...
        <HubSection
          id="therapist-productivity"
          title="Therapist Productivity Dashboard"
          subtitle="How therapist capacity, patient visits, billable units, and revenue per visit drive clinic economics."
        >
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
              <h3 className="font-display font-semibold text-ink">Therapist Productivity Funnel</h3>
              <div className="mt-6 flex flex-col items-center gap-2 text-sm">
                {["Therapist", "Patient Visits", "Billable Units", "Revenue"].map((step, i, arr) => (
                  <div key={step} className="flex flex-col items-center">
                    <span className="rounded-lg border border-border bg-surface-muted/50 px-6 py-3 font-semibold text-ink">
                      {step}
                    </span>
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-surface">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {therapistProductivityMetrics.map((row, i) => (
                    <tr
                      key={row.metric}
                      className={i < therapistProductivityMetrics.length - 1 ? "border-b border-border-subtle" : ""}
                    >
                      <td className="px-5 py-4 text-ink-muted">{row.metric}</td>
                      <td className="px-5 py-4 text-right font-semibold tabular-nums text-ink">{row.value}</td>
                    </tr>
        ...
        <HubSection
          id="visit-economics"
          title="Visit Economics"
          subtitle="The patient journey from initial evaluation through treatment visits, discharge, and outcomes."
          className="bg-surface-muted/50"
        >
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
              <h3 className="font-display font-semibold text-ink">Patient Journey</h3>
              <div className="mt-6 flex flex-col items-center gap-2 text-sm">
                {["Initial Evaluation", "Treatment Visits", "Discharge", "Outcome"].map((step, i, arr) => (
                  <div key={step} className="flex flex-col items-center">
                    <span className="rounded-lg border border-border bg-surface-muted/50 px-6 py-3 font-semibold text-ink">
                      {step}
                    </span>
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-surface">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {visitEconomicsMetrics.map((row, i) => (
                    <tr
                      key={row.metric}
                      className={i < visitEconomicsMetrics.length - 1 ? "border-b border-border-subtle" : ""}
                    >
                      <td className="px-5 py-4 text-ink-muted">{row.metric}</td>
                      <td className="px-5 py-4 text-right font-semibold tabular-nums text-ink">{row.value}</td>
                    </tr>
        ...
        <HubSection
          ...
          subtitle="Typical investment ranges for opening or acquiring a physical therapy clinic."
        >
          ...
          <p className="mt-6 text-sm text-ink-muted">
            Total startup range: $215K – $505K · De novo buildout vs acquisition varies significantly.
          </p>
        </HubSection>
        ...
        <HubSection
          ...
          subtitle="Owner compensation by clinic model — solo, multi-therapist, multi-location, and sports PT."
          className="bg-surface-muted/50"
        >
        ...
        <HubSection
          ...
          subtitle="Multiples and benchmarks used to value physical therapy clinics at sale or buy-in."
        >
          ...
          <p className="mt-6 text-sm text-ink-muted">
            Example: $850K revenue · $240K SDE → ~$770K value at 3.2× SDE
          </p>
        </HubSection>
        ...
        <HubSection
          ...
          subtitle="How referrals, patients, visits, and revenue drive practice value growth."
          className="bg-surface-muted/50"
        >
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
              ...
              <div className="mt-6 flex flex-col items-center gap-2 text-sm">
                {["Referrals", "Patients", "Visits", "Revenue", "Practice Value"].map((step, i, arr) => (
                  <div key={step} className="flex flex-col items-center">
                    <span className="rounded-lg border border-border bg-surface-muted/50 px-6 py-3 font-semibold text-ink">
                      {step}
                    </span>
        ...
        <HubSection
          id="comparisons"
          title="PT Clinic Comparisons"
          subtitle="Side-by-side economics for major physical therapy business decisions."
        >
        ...
        <HubSection
          id="popular-metrics"
          title="Most Popular PT Benchmarks"
          subtitle="High-traffic benchmarks for physical therapy clinic operators and buyers."
        >
        ...
        <HubSection
          ...
          subtitle="Featured insights on the metrics that matter most for PT clinic owners."
          className="bg-surface-muted/50"
        >
        ...
        >
          <DataSourcesAttribution attribution={physicalTherapyAttribution} />
        </HubSection>
