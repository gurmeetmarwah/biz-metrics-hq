...
import {
  ...
  physicalTherapyReportSlugs,
} from "@/lib/industries/healthcare/physical-therapy-reports";
...
export function generateStaticParams() {
  return physicalTherapyReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = physicalTherapyReports[slug];
...
export default async function PhysicalTherapyClinicReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = physicalTherapyReports[slug];
  if (!report) notFound();
        ...
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/healthcare/physical-therapy-clinic/" className="hover:text-accent">
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              >
                ← Back to Physical Therapy Clinic Hub
              </Link>
              <Link
                href="/industries/healthcare/physical-therapy-clinic/#benchmark"
