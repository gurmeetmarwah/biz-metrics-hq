...
import { treeServiceReports, treeServiceReportSlugs } from "@/lib/industries/tree-service-reports";
...
export function generateStaticParams() {
  return treeServiceReportSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const report = treeServiceReports[slug];
  if (!report) return { title: "Report Not Found | BizMetricsHQ" };

  return {
    title: `${report.meta.title} — 2026 Tree Service Industry Report | BizMetricsHQ`,
    description: report.meta.description,
  };
}

export default async function TreeServiceBenchmarkReportPage({ params }: PageProps) {
  const { slug } = await params;
  const report = treeServiceReports[slug];
  if (!report) notFound();
  ...
  const siblingReports = treeServiceReportSlugs
    .filter((s) => s !== slug)
    .map((s) => treeServiceReports[s].meta);
      ...
      <main>
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/tree-service/" className="hover:text-accent">
                Tree Service
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
            ...
            <p className="mt-6 text-xs font-semibold tracking-wider text-accent uppercase">
              Home Services Rankings · {meta.readTime} read
            </p>
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <h2 className="font-display text-lg font-semibold text-ink">Related Tree Service Reports</h2>
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/industries/tree-service/#rankings"
              ...
              >
                ← Back to Home Services Rankings
              </Link>
              <Link
                href="/calculators/tree-service-valuation/"
              ...
              >
                Tree Service Valuation Calculator
              </Link>
            </div>
          </div>
        </section>
