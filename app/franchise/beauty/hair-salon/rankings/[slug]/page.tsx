...
import {
  hairSalonFranchiseRankings,
  hairSalonFranchiseRankingSlugs,
} from "@/lib/franchise/beauty/hair-salon-rankings";
...
export function generateStaticParams() {
  return hairSalonFranchiseRankingSlugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const ranking = hairSalonFranchiseRankings[slug];
...
function extractFaqEntities(sections: (typeof hairSalonFranchiseRankings)[string]["sections"]) {
...
export default async function HairSalonFranchiseRankingPage({ params }: PageProps) {
  const { slug } = await params;
  const ranking = hairSalonFranchiseRankings[slug];
  if (!ranking) notFound();
  ...
  const siblingRankings = hairSalonFranchiseRankingSlugs
    .filter((s) => s !== slug)
    .map((s) => hairSalonFranchiseRankings[s].meta);
      ...
      <main>
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <h2 className="font-display text-lg font-semibold text-ink">More Hair Salon Franchise Rankings</h2>
            <p className="mt-2 text-sm text-ink-muted">
              Continue research with related long-tail rankings buyers search before buying a salon franchise.
            </p>
        ...
        <section className="bg-gradient-to-b from-accent-soft/40 to-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 text-center sm:px-6">
            <h2 className="font-display text-2xl font-bold tracking-tight text-ink">
              Ready to compare salon franchise opportunities?
            </h2>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <Link
                href="/franchise/beauty/hair-salon/#rankings"
