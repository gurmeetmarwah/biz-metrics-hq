...
import { hubMeta } from "@/lib/industries/hvac-hub";
import {
  reportMeta,
  reportSections,
} from "@/lib/industries/hvac-industry-report-2026";

export const metadata: Metadata = {
  title: `${reportMeta.title} — Market Size, Trends & Outlook | BizMetricsHQ`,
  description: reportMeta.description,
};

export default function HvacBenchmarksReportPage() {
  const articleJsonLd = {
      ...
      <main>
        <div className="border-b border-border/60 bg-surface">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="text-sm text-ink-faint">
              ...
              <Link href="/industries/hvac/" className="hover:text-accent">
                HVAC
              </Link>
              <span className="mx-2" aria-hidden="true">
                /
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            <DataSourcesAttribution attribution={hvacAttribution} showMacroSources />
            <p className="mt-6 text-sm text-ink-muted">
              Featured report macro figures cross-referenced against:{" "}
              {reportMeta.sources.join(" · ")}. Operator-level benchmarks validated
              against the BizMetricsHQ HVAC panel ({hubMeta.sampleSize}).
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Link
                ...
                className="inline-flex items-center rounded-xl border border-border bg-surface px-5 py-2.5 text-sm font-semibold text-ink transition-colors hover:border-accent hover:text-accent"
              >
                Revenue Per Technician Calculator
              </Link>
            </div>
          </div>
        </section>
