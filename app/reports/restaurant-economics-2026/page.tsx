...
import { restaurantMeta } from "@/lib/industries/restaurant";
import {
  reportMeta,
  reportSections,
} from "@/lib/industries/restaurant-economics-report-2026";

export const metadata: Metadata = {
  title: `${reportMeta.title} — Sales, Margins & Outlook | BizMetricsHQ`,
  description: reportMeta.description,
};

export default function RestaurantEconomicsReportPage() {
  const articleJsonLd = {
        ...
        <section className="border-t border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
            ...
            <p className="mt-3 text-sm leading-relaxed text-ink-muted">
              Macro figures and industry-wide statistics in this report are cross-referenced against
              the following sources. Operator-level benchmarks are validated against the BizMetricsHQ
              independent restaurant panel ({restaurantMeta.sampleSize}).
            </p>
            <ul className="mt-4 space-y-2 text-sm text-ink-muted">
              {reportMeta.sources.map((source) => (
