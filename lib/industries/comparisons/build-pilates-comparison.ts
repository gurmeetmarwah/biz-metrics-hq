...
import { pilatesCalculators, pilatesHubLabels } from "./pilates-comparison-shared";

type BuildPilatesComparisonOptions = {
  slug: string;
  title: string;
  subtitle: string;
...
export function buildPilatesComparison(options: BuildPilatesComparisonOptions): HealthcareDualComparisonData {
  const left = pilatesStudioProfile;
  const { right } = options;
  ...
  return {
    ...
    kpiComparison: [
      ...
      { metric: "Revenue Per Client", left: left.kpis.revenuePerMember, right: right.kpis.revenuePerMember },
    ...
    valuationExampleTitle: "$650K+ Revenue Business → Estimated Value",
    breakEvenComparison: options.breakEvenComparison,
    scalingLadder: {
      left: [...left.scalingLadder],
      right: [...right.scalingLadder],
    },
    scalingLeftTitle: "Pilates Studio Growth Path",
    ...
    chooseLeftTitle: "Choose Pilates Studio If",
