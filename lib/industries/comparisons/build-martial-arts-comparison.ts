...
import { martialArtsCalculators, martialArtsHubLabels } from "./martial-arts-comparison-shared";

type BuildMartialArtsComparisonOptions = {
  slug: string;
  title: string;
  subtitle: string;
...
export function buildMartialArtsComparison(
  options: BuildMartialArtsComparisonOptions,
): HealthcareDualComparisonData {
  const left = martialArtsSchoolProfile;
  const { right } = options;
  ...
  return {
    ...
    kpiComparison: [
      ...
      { metric: "Revenue Per Student", left: left.kpis.revenuePerMember, right: right.kpis.revenuePerMember },
    ...
    startupSideBySide: [
      {
        category: "Equipment & Buildout",
        left: left.startupSideBySide.equipment,
        right: right.startupSideBySide.equipment,
      },
      { category: "Lease & Deposits", left: left.startupSideBySide.lease, right: right.startupSideBySide.lease },
      {
        category: "Marketing Launch",
        left: left.startupSideBySide.marketing,
        right: right.startupSideBySide.marketing,
      },
    ...
    valuationExampleTitle: "$550K+ Revenue Business → Estimated Value",
    breakEvenComparison: options.breakEvenComparison,
    scalingLadder: {
      left: [...left.scalingLadder],
      right: [...right.scalingLadder],
    },
    scalingLeftTitle: "Martial Arts School Growth Path",
    ...
    chooseLeftTitle: "Choose Martial Arts School If",
    chooseRightTitle: options.chooseRightTitle,
    calculators: martialArtsCalculators,
