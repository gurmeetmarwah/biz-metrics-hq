...
import { yogaCalculators, yogaHubLabels } from "./yoga-comparison-shared";

type BuildYogaComparisonOptions = {
  slug: string;
  title: string;
  subtitle: string;
...
export function buildYogaComparison(options: BuildYogaComparisonOptions): HealthcareDualComparisonData {
  const left = yogaStudioProfile;
  const { right } = options;
  ...
  return {
    ...
    valuationExampleTitle: "$450K+ Revenue Business → Estimated Value",
    breakEvenComparison: options.breakEvenComparison,
    scalingLadder: {
      left: [...left.scalingLadder],
      right: [...right.scalingLadder],
    },
    scalingLeftTitle: "Yoga Studio Growth Path",
    ...
    chooseLeftTitle: "Choose Yoga Studio If",
    chooseRightTitle: options.chooseRightTitle,
    calculators: yogaCalculators,
