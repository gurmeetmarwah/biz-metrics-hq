...
import { getFitnessDecisionConfig } from "@/lib/industries/comparisons/fitness-decision-config";
...
export function FitnessDualDecisionTool({ slug }: { slug: string }) {
  const config = getFitnessDecisionConfig(slug);
    ...
    <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
      ...
      <div className="mt-8 space-y-6">
        <OptionGroup
          ...
          options={[
            { value: "general", label: "Full-service gym" },
            { value: "specialty", label: "Specialty / boutique studio" },
          ]}
        />
        <OptionGroup
          label="Revenue Goal"
          ...
          options={[
            { value: "moderate", label: "$400K – $700K" },
            { value: "high", label: "$700K – $1.2M" },
            { value: "very-high", label: "$1.2M+" },
          ]}
        />
        <OptionGroup
          ...
          options={[
            { value: "low", label: "Low — variety matters more" },
            { value: "medium", label: "Medium — balanced model" },
            { value: "high", label: "High — membership MRR is key" },
          ]}
        />
        <OptionGroup
          ...
          options={[
            { value: "lifestyle", label: "Single-location lifestyle" },
            { value: "regional", label: "Regional multi-location" },
