...
import { BeautyDualDecisionTool } from "@/components/comparison/BeautyDualDecisionTool";
import type { BeautyDualComparisonData } from "@/lib/industries/comparisons/beauty-comparison-types";
...
export function BeautyDualComparisonPage({ data }: { data: BeautyDualComparisonData }) {
  ...
  return (
    <>
      ...
      <main>
        ...
        <Section id="revenue-comparison" title="Revenue Comparison Center" subtitle="How each model turns visits into revenue." className="bg-surface-muted/50">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6">
        ...
        <Section id="client-economics" title="Client Economics Dashboard" subtitle="Lifetime value and visit economics — the core financial differentiator.">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6">
              <h3 className="font-display font-semibold text-metric-salary">{left}</h3>
              <div className="mt-6">
                <FlowSteps steps={data.clientFunnel.left} />
              </div>
            </div>
            <div className="rounded-2xl border border-border bg-surface p-6">
              <h3 className="font-display font-semibold text-metric-margin">{right}</h3>
              <div className="mt-6">
                <FlowSteps steps={data.clientFunnel.right} />
              </div>
            </div>
          </div>
          <div className="mt-8">
            ...
            <div className="mt-4">
              <ComparisonTable
                ...
                rows={data.clientEconomicsMetrics.map((r) => ({ label: r.metric, left: r.left, right: r.right }))}
              />
            </div>
          </div>
        </Section>

        <Section id="service-economics" title="Chair & Service Economics" subtitle="Revenue per chair and provider productivity." className="bg-surface-muted/50">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6">
              <h3 className="font-display font-semibold text-metric-salary">{left}</h3>
              <div className="mt-6">
                <FlowSteps steps={data.serviceFunnel.left} />
              </div>
            </div>
            <div className="rounded-2xl border border-border bg-surface p-6">
              <h3 className="font-display font-semibold text-metric-margin">{right}</h3>
              <div className="mt-6">
                <FlowSteps steps={data.serviceFunnel.right} />
              </div>
            </div>
          </div>
          <div className="mt-8">
            <ComparisonTable
              headers={["Metric", left, right]}
              rows={data.serviceMetrics.map((r) => ({ label: r.metric, left: r.left, right: r.right }))}
            />
          </div>
        </Section>
        ...
        <Section id="recurring-retail" title="Recurring & Retail Mix" subtitle="Rebooking, memberships, and retail drive predictable revenue." className="bg-surface-muted/50">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6 text-center">
              <p className="font-display font-semibold text-metric-salary">{left}</p>
              <p className="mt-2 text-sm text-ink-muted">{data.recurringHighlights.left.title}</p>
              <p className="mt-3 text-2xl font-bold tabular-nums text-ink">{data.recurringHighlights.left.value}</p>
            </div>
            <div className="rounded-2xl border border-border bg-surface p-6 text-center">
              <p className="font-display font-semibold text-metric-margin">{right}</p>
              <p className="mt-2 text-sm text-ink-muted">{data.recurringHighlights.right.title}</p>
              <p className="mt-3 text-2xl font-bold tabular-nums text-ink">{data.recurringHighlights.right.value}</p>
            </div>
          </div>
          <div className="mt-8">
            <ComparisonTable
              headers={["Metric", left, right]}
              rows={data.recurringComparison.map((r) => ({ label: r.metric, left: r.left, right: r.right }))}
            />
          </div>
        </Section>
        ...
        <Section id="startup-costs" title="Startup Cost Comparison" subtitle="Investment required to launch each model." className="bg-surface-muted/50">
        ...
        <Section id="valuation" title="Valuation Comparison">
          ...
          <div className="mt-8 rounded-2xl border border-border bg-surface-muted/50 p-6">
            <p className="text-center text-sm font-semibold text-ink">
              {data.valuationExampleTitle ?? "Typical Exit Outcomes"}
            </p>
        ...
        <Section id="decision-tool" title="Interactive Decision Tool" className="bg-surface-muted/50">
          <BeautyDualDecisionTool slug={data.comparisonMeta.slug} />
        </Section>
