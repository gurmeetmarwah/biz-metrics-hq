...
import { FoodTruckPrimaryDecisionTool } from "@/components/comparison/FoodTruckPrimaryDecisionTool";

type StartupItem = { item: string; percent: number };
type Faq = { question: string; answer: string };
type LinkItem = { label: string; href?: string; comingSoon?: boolean };

export type FoodTruckPrimaryComparisonData = {
  comparisonMeta: {
    slug: "food-truck-vs-catering" | "food-truck-vs-ghost-kitchen";
    title: string;
    subtitle: string;
    dataVintage: string;
  };
  comparisonLabels: {
    ...
    explorePrimary: { label: string; href: string };
    exploreFoodTruck: { label: string; href: string };
  };
  decisionSnapshot: readonly { factor: string; winner: string }[];
  kpiComparison: readonly { metric: string; primary: string; foodTruck: string }[];
  scorecardCategories: readonly { label: string; primary: number; foodTruck: number }[];
  primaryStartupItems: readonly StartupItem[];
  foodTruckStartupItems: readonly StartupItem[];
  startupSideBySide: readonly { category: string; primary: string; foodTruck: string }[];
  primaryRevenueDist: { bottom: string; median: string; top: string };
  foodTruckRevenueDist: { bottom: string; median: string; top: string };
  primaryRevenueDrivers: readonly string[];
  foodTruckRevenueDrivers: readonly string[];
  marginTiers: {
    primary: { weak: string; average: string; strong: string };
    foodTruck: { weak: string; average: string; strong: string };
  };
  costStructureComparison: readonly { expense: string; primary: string; foodTruck: string }[];
  breakEvenComparison: {
    primaryMonths: number;
    foodTruckMonths: number;
    primaryMonthsLabel: string;
    foodTruckMonthsLabel: string;
  };
  dailySalesComparison: readonly { metric: string; primary: string; foodTruck: string }[];
  ownerIncomeComparison: readonly {
    model: string;
    earnings: string;
    type: "primary" | "foodTruck";
  }[];
  lifestyleComparison: readonly { factor: string; primary: string; foodTruck: string }[];
  scalingLadder: { primary: string[]; foodTruck: string[] };
  scalingChallenges: readonly { title: string; description: string }[];
  valuationComparison: readonly { metric: string; primary: string; foodTruck: string }[];
  valuationExamples: {
    primary: { revenue: string; value: string; note: string };
    foodTruck: { revenue: string; value: string; note: string };
  };
  choosePrimary: readonly string[];
  chooseFoodTruck: readonly string[];
...
export function FoodTruckPrimaryComparisonPage({
  data,
}: {
  data: FoodTruckPrimaryComparisonData;
}) {
  const {
    comparisonMeta,
    comparisonLabels,
  ...
  const { primary, foodTruck } = comparisonLabels;
  const maxBreakEvenMonths = Math.max(
    breakEvenComparison.primaryMonths,
    breakEvenComparison.foodTruckMonths,
    24
  );

  const scorecardForComponent = scorecardCategories.map((cat) => ({
    label: cat.label,
    restaurant: cat.primary,
    foodTruck: cat.foodTruck,
  }));
      ...
      <main>
        <section className="hero-mesh border-b border-border/60">
          <div className="mx-auto max-w-4xl px-4 py-12 text-center sm:px-6 sm:py-16">
            <nav aria-label="Breadcrumb" className="mb-6 text-left text-sm text-ink-faint">
              ...
              <Link href={comparisonLabels.breadcrumbHub.href} className="hover:text-accent">
                ...
                <tbody>
                  {decisionSnapshot.map((row, i) => (
                    <tr
                      key={row.factor}
                      className={
                        i < decisionSnapshot.length - 1 ? "border-b border-border-subtle" : ""
                      }
                    >
              ...
              <table className="w-full min-w-[480px] text-sm">
                ...
                <tbody>
                  {kpiComparison.map((row, i) => (
                    <tr
                      key={row.metric}
                      className={i < kpiComparison.length - 1 ? "border-b border-border-subtle" : ""}
                    >
        ...
        <section className="border-b border-border/60 bg-surface-muted/50">
          <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
            ...
            <div className="mt-8">
              <ComparisonScorecard
                categories={scorecardForComponent}
                restaurantLabel={primary}
                foodTruckLabel={foodTruck}
              />
            </div>
          </div>
        </section>
        ...
        <Section
          ...
          subtitle={`One of the biggest factors in the ${primary} vs ${foodTruck} decision.`}
        >
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <StartupVisual
              title={`${primary} Startup Costs`}
              items={primaryStartupItems}
              color="var(--metric-margin)"
            />
            <StartupVisual
              title={`${foodTruck} Startup Costs`}
              ...
              <tbody>
                {startupSideBySide.map((row, i) => (
                  <tr
                    key={row.category}
                    className={
                      i < startupSideBySide.length - 1 ? "border-b border-border-subtle" : ""
                    }
                  >
        ...
        <Section
          ...
          subtitle={`How ${primary} and ${foodTruck} models generate and scale revenue.`}
          className="bg-surface-muted/50"
        >
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <RevenueDistBar
              label={primary}
              bottom={primaryRevenueDist.bottom}
              median={primaryRevenueDist.median}
              top={primaryRevenueDist.top}
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6">
              <h3 className="font-display font-semibold text-ink">{primary} Revenue Drivers</h3>
              <ul className="mt-4 space-y-2">
                {primaryRevenueDrivers.map((d) => (
            ...
            <div className="rounded-2xl border border-border bg-surface p-6">
              <h3 className="font-display font-semibold text-ink">{foodTruck} Revenue Drivers</h3>
              <ul className="mt-4 space-y-2">
        ...
        >
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <MarginGauge label={`${primary} Margins`} tiers={marginTiers.primary} />
            <MarginGauge label={`${foodTruck} Margins`} tiers={marginTiers.foodTruck} />
          </div>
          <div className="mt-8 overflow-hidden rounded-2xl border border-border">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {costStructureComparison.map((row, i) => (
                  <tr
                    key={row.expense}
                    className={
                      i < costStructureComparison.length - 1 ? "border-b border-border-subtle" : ""
                    }
                  >
                    <td className="px-5 py-4 text-ink-muted sm:px-6">{row.expense}</td>
                    <td className="px-5 py-4 text-right font-semibold sm:px-6">{row.primary}</td>
        ...
        >
          <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
            ...
            <div className="mt-6 space-y-6">
              <div>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="tabular-nums text-ink-muted">
                    {breakEvenComparison.primaryMonthsLabel}
                  </span>
                </div>
                <div className="mt-2 h-4 overflow-hidden rounded-full bg-surface-muted">
                  <div
                    ...
                    style={{
                      width: `${(breakEvenComparison.primaryMonths / maxBreakEvenMonths) * 100}%`,
                    }}
                  />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between text-sm">
                  <span className="font-medium text-ink">{foodTruck}</span>
                  <span className="tabular-nums text-ink-muted">
              ...
              <tbody>
                {dailySalesComparison.map((row, i) => (
                  <tr
                    key={row.metric}
                    className={
                      i < dailySalesComparison.length - 1 ? "border-b border-border-subtle" : ""
                    }
                  >
              ...
              >
                ...
                <p
                  className={`mt-2 text-2xl font-bold tabular-nums ${
                    item.type === "primary" ? "text-metric-margin" : "text-metric-salary"
                  }`}
                >
                  {item.earnings}
                </p>
              ...
              <tbody>
                {lifestyleComparison.map((row, i) => (
                  <tr
                    key={row.factor}
                    className={
                      i < lifestyleComparison.length - 1 ? "border-b border-border-subtle" : ""
                    }
                  >
                    <td className="px-5 py-4 font-medium text-ink sm:px-6">{row.factor}</td>
                    <td className="px-5 py-4 text-ink-muted sm:px-6">{row.primary}</td>
        ...
        >
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-border bg-surface p-6">
              <p className="font-display font-semibold text-metric-salary">{foodTruck}</p>
            ...
            <div className="rounded-2xl border border-border bg-surface p-6">
              <p className="font-display font-semibold text-metric-margin">{primary}</p>
              <ul className="mt-4 space-y-3">
                {scalingLadder.primary.map((step, i) => (
                  <li key={step} className="flex items-center gap-3">
          ...
          <div className="mt-10">
            <h3 className="font-display text-lg font-semibold text-ink">Scaling Challenges</h3>
        ...
        >
              ...
              <tbody>
                {valuationComparison.map((row, i) => (
                  <tr
                    key={row.metric}
                    className={
                      i < valuationComparison.length - 1 ? "border-b border-border-subtle" : ""
                    }
                  >
                    <td className="px-5 py-4 text-ink-muted sm:px-6">{row.metric}</td>
                    <td className="px-5 py-4 text-right font-semibold sm:px-6">{row.primary}</td>
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
            {[
              { label: primary, ...valuationExamples.primary },
              { label: foodTruck, ...valuationExamples.foodTruck },
            ].map((ex) => (
              <div
                key={ex.label}
        ...
        >
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-metric-salary/25 bg-metric-salary/5 p-6">
              <h3 className="font-display font-semibold text-ink">Choose a {foodTruck} If</h3>
              <ul className="mt-4 space-y-3">
              ...
              <Link
                href={comparisonLabels.exploreFoodTruck.href}
                className="mt-6 inline-block text-sm font-medium text-accent hover:underline"
              >
                {comparisonLabels.exploreFoodTruck.label}
              </Link>
            </div>
            <div className="rounded-2xl border border-metric-margin/25 bg-metric-margin/5 p-6">
              <h3 className="font-display font-semibold text-ink">Choose {primary} If</h3>
              <ul className="mt-4 space-y-3">
                {choosePrimary.map((item) => (
                  <li key={item} className="flex items-start gap-2.5 text-sm text-ink-muted">
                    <span className="font-bold text-metric-revenue">✓</span>
                    {item}
                  </li>
              ...
              <Link
                href={comparisonLabels.explorePrimary.href}
                className="mt-6 inline-block text-sm font-medium text-accent hover:underline"
              >
                {comparisonLabels.explorePrimary.label}
              </Link>
            </div>
          </div>
        </Section>
        ...
        <Section
          ...
          className="bg-surface-muted/50"
        >
          <FoodTruckPrimaryDecisionTool slug={comparisonMeta.slug} />
        </Section>

        <Section id="calculators" title="Comparison Calculators" subtitle="Run the numbers on each model.">
