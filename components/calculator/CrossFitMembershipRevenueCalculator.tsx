...
import {
  ...
  relatedTools,
} from "@/lib/industries/crossfit-membership-revenue-calculator";

export function CrossFitMembershipRevenueCalculator() {
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Implied Box Revenue</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.impliedBoxRevenue)}</p>
              </div>
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">Median ~{formatCurrency(crossfitIndustryAverages.mrr)}/mo MRR</p>
                <p className="mt-1 text-xs text-ink-faint">
                  {result.vsBenchmarkMrr >= 0 ? "+" : ""}
                  {formatCurrency(result.vsBenchmarkMrr)} vs median · ~{crossfitIndustryAverages.activeMembers} active members
                </p>
              </div>
            </div>
          </div>
