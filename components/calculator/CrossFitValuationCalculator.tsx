...
import {
  ...
  relatedTools,
} from "@/lib/industries/crossfit-valuation-calculator";

export function CrossFitValuationCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const [sdeManual, setSdeManual] = useState(false);
  const result = useMemo(() => calculateCrossfitValuation(inputs), [inputs]);
  ...
  const update = <K extends keyof typeof inputs>(key: K, value: (typeof inputs)[K]) => {
    ...
    setInputs((prev) => {
      ...
      if (key === "netProfit" || key === "ownerSalary") {
        ...
        next.sde = Math.round(next.revenue * 0.23);
      }
      return next;
    });
  };
            ...
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                <PercentInput
                  ...
                  value={inputs.membershipRevenuePct}
                  onChange={(v) => update("membershipRevenuePct", v)}
                />
              </div>
            </div>
            <div className="flex-1">
              <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8 text-center">
                ...
                <p className="mt-2 text-xs text-ink-faint">
                  At {formatMultiple(result.adjustedMultiple)} SDE (adjusted for {inputs.membershipRevenuePct}% membership revenue)
                </p>
              </div>
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          ...
          <p className="mt-6 text-sm text-ink-muted">
            Example: {formatCurrency(crossfitIndustryAverages.revenue)} revenue · {formatCurrency(crossfitIndustryAverages.sde)} SDE → ~
            {formatCurrency(crossfitIndustryAverages.sde * crossfitIndustryAverages.sdeMultiple)} at {formatMultiple(crossfitIndustryAverages.sdeMultiple)} SDE
          </p>
        </div>
      </section>
