...
import {
  ...
  relatedTools,
} from "@/lib/industries/roofing-crew-productivity-calculator";

export function RoofingCrewProductivityCalculator() {
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <p className="mt-1 text-sm text-ink-muted">Measure install throughput and revenue per crew.</p>
              <div className="mt-6 space-y-4">
                ...
                <label htmlFor="months" className="block">
                  <span className="text-sm font-medium text-ink-muted">Working Months Per Year</span>
                  <input
                    id="months"
                    type="number"
                    min={6}
                    max={12}
                    value={inputs.workingMonths}
                    onChange={(e) => update("workingMonths", Number(e.target.value) || 10)}
                    className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10"
                  />
                </label>
                <label htmlFor="utilization" className="block">
                  ...
                  <input
                    ...
                    min={30}
                    max={100}
                    value={inputs.crewUtilizationPct}
                    onChange={(e) => update("crewUtilizationPct", Number(e.target.value) || 72)}
                    className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10"
                  />
                </label>
              </div>
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                ...
                <p className="mt-1 text-4xl font-bold tabular-nums text-metric-revenue">
                  {formatCurrency(result.revenuePerCrew)}
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Revenue Per Install</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-accent">
                  {formatCurrency(result.revenuePerInstall)}
                </p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Installs Per Crew / Year</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">
                  {formatNumber(result.installsPerCrewPerYear, 0)}
                </p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Crew Utilization</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">
                  {formatPercent(inputs.crewUtilizationPct, 0)}
                </p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-ink-faint">
                  Median {formatCurrency(roofingIndustryAverages.revenuePerCrew)} per crew · 310+ Roofing businesses
                </p>
              </div>
            </div>
          </div>
