...
import { PilatesCalculatorFooter } from "@/components/calculator/PilatesCalculatorFooter";
...
import {
  ...
  relatedTools,
} from "@/lib/industries/pilates-client-lifetime-value-calculator";

export function PilatesClientLifetimeValueCalculator() {
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                <CurrencyInput
                  id="rpc"
                  label="Monthly Revenue Per Client"
                  value={inputs.monthlyRevenuePerClient}
                  onChange={(v) => update("monthlyRevenuePerClient", v)}
                />
                <PercentInput
                  id="churn"
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                ...
                <p className="mt-2 text-sm text-ink-faint">
                  {result.tier} vs benchmark · {formatCurrency(result.vsBenchmark)} vs ~{formatCurrency(pilatesIndustryAverages.clientLtv)} median
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-ink-faint">~{formatCurrency(pilatesIndustryAverages.revenuePerClientMonthly)}/mo revenue per client</p>
              </div>
            </div>
          </div>
        </div>
