...
import { YogaCalculatorFooter } from "@/components/calculator/YogaCalculatorFooter";
...
import {
  ...
  relatedTools,
} from "@/lib/industries/yoga-client-lifetime-value-calculator";

export function YogaClientLifetimeValueCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateClientLtv(inputs), [inputs]);
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              <h2 className="font-display text-lg font-semibold text-ink">Client Economics</h2>
              <p className="mt-1 text-sm text-ink-muted">Model LTV from revenue per client and churn.</p>
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                ...
                <p className="mt-2 text-sm text-ink-faint">
                  {result.tier} vs benchmark · {formatCurrency(result.vsBenchmark)} vs ~{formatCurrency(yogaIndustryAverages.clientLtv)} median
                </p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Avg. Client Lifespan</p>
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-ink-faint">~{formatCurrency(yogaIndustryAverages.revenuePerMemberMonthly)}/mo revenue per member</p>
              </div>
            </div>
          </div>
        </div>
