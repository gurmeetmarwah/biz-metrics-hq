...
import {
  ...
  multipleRanges,
  relatedTools,
} from "@/lib/industries/gym-valuation-calculator";

export function GymValuationCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const [sdeManual, setSdeManual] = useState(false);
  const result = useMemo(() => calculateGymValuation(inputs), [inputs]);
              ...
              <div className="mt-6 space-y-4">
                ...
                <label htmlFor="multiple" className="block">
                  ...
                  <input
                    ...
                    onChange={(e) => update("sdeMultiple", Number(e.target.value) || 3.5)}
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          ...
          <p className="mt-6 text-sm text-ink-muted">
            Example: {formatCurrency(gymIndustryAverages.revenue)} revenue · {formatCurrency(gymIndustryAverages.sde)} SDE → ~
            {formatCurrency(gymIndustryAverages.sde * gymIndustryAverages.sdeMultiple)} at {formatMultiple(gymIndustryAverages.sdeMultiple)} SDE
          </p>
        </div>
      </section>
