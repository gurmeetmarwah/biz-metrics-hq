...
export function CoffeeShopRevenueCalculator() {
    ...
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                <label className="block">
                  ...
                  <input type="number" min={0} step={0.25} value={inputs.averageTicket} onChange={(e) => update("averageTicket", Number(e.target.value) || 0)}
                    className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10" />
                </label>
                <label className="block">
                ...
                <label className="block">
                  <span className="text-sm font-medium text-ink-muted">Food Attach Rate (%)</span>
                  <input type="number" min={0} max={100} value={inputs.foodAttachPct} onChange={(e) => update("foodAttachPct", Number(e.target.value) || 0)}
                    className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10" />
                </label>
              </div>
            </div>
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2">
                ...
                <p className="mt-2 inline-flex rounded-full border border-accent/25 bg-accent-soft px-3 py-1 text-sm font-semibold text-accent">
                  {result.tier}
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Beverage Revenue</p>
                <p className="mt-1 text-xl font-bold tabular-nums text-ink">{formatCurrency(result.beverageRevenue)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Food Revenue</p>
                <p className="mt-1 text-xl font-bold tabular-nums text-ink">{formatCurrency(result.foodRevenue)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2">
                <p className="text-sm text-ink-muted">Industry Median: {formatCurrency(coffeeShopIndustryAverages.revenue)}</p>
                <p className="mt-1 text-sm text-ink-muted">
                  Annual transactions: {formatNumber(result.transactionsPerYear, 0)}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <h2 className="font-display text-xl font-semibold text-ink">Day Scenarios</h2>
          <div className="mt-4 h-0.5 w-10 rounded-full bg-accent" aria-hidden="true" />
          <ul className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {scenarios.map((s) => {
              const annual = s.customersPerDay * s.averageTicket * inputs.daysOpenPerYear;
              return (
                <li key={s.label} className="rounded-2xl border border-border bg-surface p-5">
                  <p className="font-semibold text-ink">{s.label}</p>
                  <p className="mt-1 text-sm text-ink-muted">
                    {s.customersPerDay} customers × ${s.averageTicket.toFixed(2)}
                  </p>
