...
export function CoffeeShopFoodCostCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateCoffeeShopFoodCost(inputs), [inputs]);
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              <h2 className="font-display text-lg font-semibold text-ink">Monthly Food & Beverage Sales</h2>
              <div className="mt-6 space-y-4">
                ...
                <CurrencyInput
                  id="cs-beans"
                  label="Coffee & Bean Costs"
                  value={inputs.coffeeBeanCosts}
                  onChange={(v) => update("coffeeBeanCosts", v)}
                />
                <CurrencyInput
                  id="cs-dairy"
                  label="Dairy & Alternative Costs"
                  value={inputs.dairyCosts}
                  onChange={(v) => update("dairyCosts", v)}
                />
                <CurrencyInput
                  id="cs-pastry"
                  label="Pastry & Food Costs"
                  value={inputs.pastryFoodCosts}
                  onChange={(v) => update("pastryFoodCosts", v)}
                />
                <CurrencyInput
                  id="cs-waste"
              ...
              <div className="rounded-2xl border border-border bg-surface-muted/30 p-6 sm:p-8">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-3xl font-bold tabular-nums text-ink">
                      {formatPercent(result.grossMarginPct)}
                    </p>
                  </div>
                </div>
                <div className="mt-6 rounded-xl border border-border bg-surface px-5 py-4">
                  ...
                  <p className="mt-1 font-semibold text-ink">
                    {coffeeShopIndustryAverages.ingredientCostPct}% typical · 18–25% total food & beverage cost
                  </p>
                </div>
              ...
              return (
                <li
                  ...
                  className={`rounded-2xl border p-5 ${
                    active ? "border-accent bg-accent-soft/50" : "border-border bg-surface"
                  }`}
                >
