...
import {
  ...
  relatedTools,
} from "@/lib/industries/yoga-valuation-calculator";

export function YogaValuationCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const [sdeManual, setSdeManual] = useState(false);
  const result = useMemo(() => calculateYogaValuation(inputs), [inputs]);
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          ...
          <p className="mt-6 text-sm text-ink-muted">
            Example: {formatCurrency(yogaIndustryAverages.revenue)} revenue · {formatCurrency(yogaIndustryAverages.sde)} SDE → ~
            {formatCurrency(yogaIndustryAverages.sde * yogaIndustryAverages.sdeMultiple)} at {formatMultiple(yogaIndustryAverages.sdeMultiple)} SDE
          </p>
        </div>
      </section>
