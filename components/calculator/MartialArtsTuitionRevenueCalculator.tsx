...
import {
  ...
  relatedTools,
  tuitionBenchmarks,
} from "@/lib/industries/martial-arts-tuition-revenue-calculator";

export function MartialArtsTuitionRevenueCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateTuitionRevenue(inputs), [inputs]);
    ...
    <>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              <h2 className="font-display text-lg font-semibold text-ink">Tuition Inputs</h2>
              <p className="mt-1 text-sm text-ink-muted">Model tuition MRR from active students and program fees.</p>
              <div className="mt-6 space-y-4">
                ...
                <PercentInput
                  id="autopay"
                  label="Autopay Students %"
                  value={inputs.autopayStudentPct}
                  onChange={(v) => update("autopayStudentPct", v)}
                />
              </div>
            </div>
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                <p className="text-sm text-ink-muted">Tuition MRR</p>
                <p className="mt-1 text-4xl font-bold tabular-nums text-metric-revenue">
                  {formatCurrency(result.tuitionMrr)}
                </p>
                <p className="mt-2 text-sm text-ink-faint">
                  {result.tier} vs benchmark · {formatCurrency(result.annualTuitionRevenue)}/yr
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Implied School Revenue</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.impliedSchoolRevenue)}</p>
              </div>
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">Median ~{formatCurrency(martialArtsIndustryAverages.tuitionMrr)}/mo tuition MRR</p>
                <p className="mt-1 text-xs text-ink-faint">
                  {result.vsBenchmarkMrr >= 0 ? "+" : ""}
                  {formatCurrency(result.vsBenchmarkMrr)} vs median · ~{martialArtsIndustryAverages.activeStudents} active students
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <h2 className="font-display text-xl font-semibold text-ink">Tuition Benchmarks</h2>
          ...
          <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-surface">
            <table className="w-full text-sm">
              ...
              <tbody>
                {tuitionBenchmarks.map((row, i) => (
                  <tr key={row.metric} className={i < tuitionBenchmarks.length - 1 ? "border-b border-border-subtle" : ""}>
