...
import {
  ...
  relatedTools,
} from "@/lib/industries/cleaning-business-contract-value-calculator";

export function CleaningBusinessContractValueCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateContractValue(inputs), [inputs]);
    ...
    <>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                <label htmlFor="accounts" className="block">
                  ...
                  <input id="accounts" type="number" min={0} max={500} value={inputs.commercialAccounts}
                ...
                <CurrencyInput id="monthly" label="Monthly Revenue Per Commercial Account" value={inputs.monthlyRevenuePerAccount}
                ...
                <CurrencyInput id="onetime" label="One-Time Monthly Revenue" value={inputs.oneTimeMonthlyRevenue}
                  onChange={(v) => update("oneTimeMonthlyRevenue", v)} />
              </div>
            </div>
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Avg. Annual Contract Value</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.avgAnnualContractValue)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">One-Time Revenue</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.annualOneTimeRevenue)}</p>
              </div>
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">{cleaningBusinessIndustryAverages.recurringRevenuePct}% median recurring revenue</p>
                <p className="mt-1 text-xs text-ink-faint">
                  {result.vsBenchmark >= 0 ? "+" : ""}{result.vsBenchmark.toFixed(0)} pts vs benchmark · 190+ cleaning businesses
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <h2 className="font-display text-xl font-semibold text-ink">Contract Value Benchmarks</h2>
