...
import {
  ...
  relatedTools,
} from "@/lib/industries/pilates-valuation-calculator";

export function PilatesValuationCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const [sdeManual, setSdeManual] = useState(false);
  const result = useMemo(() => calculatePilatesValuation(inputs), [inputs]);
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          ...
          <p className="mt-6 text-sm text-ink-muted">
            Example: {formatCurrency(pilatesIndustryAverages.revenue)} revenue · {formatCurrency(pilatesIndustryAverages.sde)} SDE → ~
            {formatCurrency(pilatesIndustryAverages.sde * pilatesIndustryAverages.sdeMultiple)} at {formatMultiple(pilatesIndustryAverages.sdeMultiple)} SDE
          </p>
        </div>
      </section>
