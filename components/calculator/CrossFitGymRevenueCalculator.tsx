...
import {
  ...
  relatedTools,
  revenueBenchmarks,
} from "@/lib/industries/crossfit-gym-revenue-calculator";

export function CrossFitGymRevenueCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateCrossfitRevenue(inputs), [inputs]);
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <p className="mt-1 text-sm text-ink-muted">Model revenue from members, PT, nutrition, and ancillary income.</p>
              <div className="mt-6 space-y-4">
                ...
                <CurrencyInput
                  id="nutrition"
                  label="Nutrition Coaching (Monthly Total)"
                  value={inputs.monthlyNutritionCoaching}
                  onChange={(v) => update("monthlyNutritionCoaching", v)}
                />
                <CurrencyInput
                  id="foundations"
                  label="Foundations & Retail (Monthly Total)"
                  value={inputs.monthlyFoundationsRetail}
                  onChange={(v) => update("monthlyFoundationsRetail", v)}
                />
                <CurrencyInput
                  id="events"
                  label="Events & Merchandise (Monthly Total)"
                  value={inputs.monthlyEventsMerch}
                  onChange={(v) => update("monthlyEventsMerch", v)}
                />
              </div>
            </div>
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">Median ~{formatCurrency(crossfitIndustryAverages.revenue)} annual revenue</p>
                <p className="mt-1 text-xs text-ink-faint">
                  {result.vsBenchmarkRevenue >= 0 ? "+" : ""}
                  {formatCurrency(result.vsBenchmarkRevenue)} vs median · ~{formatCurrency(crossfitIndustryAverages.revenuePerMemberMonthly)}/mo per member
                </p>
              </div>
            </div>
          </div>
