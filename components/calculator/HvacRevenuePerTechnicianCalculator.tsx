...
import { HvacCalculatorFooter } from "@/components/calculator/HvacCalculatorFooter";
import { hvacDataLinks } from "@/lib/industries/hvac-calculators-shared";
import {
  calculateRevenuePerTechnician,
  defaultInputs,
  faqs,
  formatCurrency,
  formatNumber,
  ...
  relatedTools,
} from "@/lib/industries/hvac-revenue-per-technician-calculator";

export function HvacRevenuePerTechnicianCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateRevenuePerTechnician(inputs), [inputs]);
  const totalJobsPerYear = inputs.technicians * inputs.workingDays * inputs.jobsPerDay;
    ...
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                />
                <label htmlFor="technicians" className="block">
                  ...
                  <input
                    ...
                    onChange={(e) => update("technicians", Number(e.target.value) || 8)}
                ...
                <label htmlFor="jobs" className="block">
                  <span className="text-sm font-medium text-ink-muted">Jobs Per Day (Per Tech)</span>
                  <input
                    id="jobs"
                    type="number"
                    min={1}
                    ...
                    onChange={(e) => update("jobsPerDay", Number(e.target.value) || 5)}
                    className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10"
                  />
                </label>
              </div>
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                ...
                <p className="mt-1 text-4xl font-bold tabular-nums text-metric-revenue">
                  {formatCurrency(result.revenuePerTechnician)}
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Daily Production Per Tech</p>
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">
                  {hvacIndustryAverages.revenuePerTechnicianRange}
                </p>
                <p className="mt-1 text-xs text-ink-faint">
                  Median {formatCurrency(hvacIndustryAverages.revenuePerTechnician)} · 420+ HVAC businesses
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <h2 className="font-display text-xl font-semibold text-ink">HVAC Productivity Benchmarks</h2>
      ...
      <HvacCalculatorFooter relatedTools={relatedTools} dataLinks={hvacDataLinks} faqs={faqs} />
    </>
  );
}
