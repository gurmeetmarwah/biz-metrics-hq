...
import {
  ...
  industryBenchmarks,
  relatedTools,
} from "@/lib/industries/hair-salon-break-even-calculator";

export function HairSalonBreakEvenCalculator() {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Appointments / Month</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatNumber(result.breakEvenAppointments)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Appointments / Day</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-accent">{formatNumber(result.appointmentsPerDay, 1)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Contribution / Ticket</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.contributionPerTicket)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      <HairSalonCalculatorFooter calculatorId="break-even" relatedTools={relatedTools} dataLinks={hairSalonDataLinks} faqs={faqs} />
    </>
  );
}
