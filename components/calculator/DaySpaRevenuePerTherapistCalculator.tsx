...
import {
  ...
  daySpaDataLinks,
  relatedTools,
} from "@/lib/industries/day-spa-revenue-per-therapist-calculator";

export function DaySpaRevenuePerTherapistCalculator() {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                <label htmlFor="therapists" className="block">
                  <span className="text-sm font-medium text-ink-muted">Number of Therapists</span>
                  <input id="therapists" type="number" min={1} value={inputs.numberOfTherapists}
                    onChange={(e) => update("numberOfTherapists", Number(e.target.value) || 1)}
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2">
                ...
                <p className="mt-1 text-4xl font-bold tabular-nums text-metric-revenue">{formatCurrency(result.perTherapist)}</p>
                <p className="mt-2 text-sm capitalize text-ink-muted">Status: {result.status}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Monthly per Therapist</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.monthlyPerTherapist)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Weekly per Therapist</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-accent">{formatCurrency(result.weeklyPerTherapist)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <DaySpaCalculatorFooter calculatorId="revenue-per-therapist" relatedTools={relatedTools} dataLinks={daySpaDataLinks} faqs={faqs} />
    </>
  );
}
