...
export function CrossFitCalculatorFooter({
  relatedTools,
  dataLinks,
  faqs,
}: {
    ...
    <>
      <section className="border-b border-border/60 bg-surface">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <h2 className="font-display text-xl font-semibold text-ink">Related CrossFit Data</h2>
            ...
            {dataLinks.map((link) => (
              <li key={link.href}>
                <HashLink
                  href={link.href}
                  prefetch={false}
                  className="card-interactive block rounded-2xl border border-border bg-surface p-5"
                >
...
export function CrossFitNumberInput({
  id,
  label,
  value,
  onChange,
