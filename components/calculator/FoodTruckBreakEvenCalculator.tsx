...
export function FoodTruckBreakEvenCalculator() {
    ...
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                <CurrencyInput id="fixed" label="Monthly Fixed Costs" value={inputs.fixedCosts} onChange={(v) => update("fixedCosts", v)} tooltip="Insurance, commissary, loan payments, etc." />
                <CurrencyInput id="ticket" label="Average Ticket" value={inputs.averageTicket} onChange={(v) => update("averageTicket", v)} />
                <PercentInput id="food" label="Food Cost %" value={inputs.foodCostPct} onChange={(v) => update("foodCostPct", v)} />
                ...
                <label htmlFor="days" className="block">
                  <span className="text-sm font-medium text-ink-muted">Operating Days Per Month</span>
                  <input id="days" type="number" min={1} max={31} value={inputs.daysOpen} onChange={(e) => update("daysOpen", Number(e.target.value) || 22)}
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                ...
                <p className="mt-1 text-2xl font-bold tabular-nums text-accent">{formatNumber(result.dailyCustomers, 0)}/day</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      <section className="border-b border-border/60 bg-surface-muted/50">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14">
          <h2 className="font-display text-xl font-semibold text-ink">Food Truck Benchmarks</h2>
      ...
      <FoodTruckCalculatorFooter calculatorId="break-even" relatedTools={relatedTools} dataLinks={foodTruckDataLinks} faqs={faqs} />
    </>
  );
}
