...
export function IceCreamShopLaborCostCalculator() {
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                <label className="block">
                  <span className="text-sm font-medium text-ink-muted">Full-Time Scoopers</span>
                  <input type="number" min={0} value={inputs.fullTimeScoopers} onChange={(e) => update("fullTimeScoopers", Number(e.target.value) || 0)}
                ...
                <label className="block">
                  <span className="text-sm font-medium text-ink-muted">Part-Time Scoopers</span>
                  <input type="number" min={0} value={inputs.partTimeScoopers} onChange={(e) => update("partTimeScoopers", Number(e.target.value) || 0)}
                    className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10" />
                </label>
                <label className="block">
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2">
                <p className="text-sm text-ink-muted">vs. Industry Average ({iceCreamShopIndustryAverages.laborCostPct}%)</p>
      ...
      <IceCreamShopCalculatorFooter calculatorId="labor-cost" relatedTools={relatedTools} dataLinks={iceCreamShopDataLinks} faqs={faqs} />
    </>
  );
}
