...
import {
  ...
  medicalSpaDataLinks,
  relatedTools,
} from "@/lib/industries/medical-spa-break-even-calculator";

export function MedicalSpaBreakEvenCalculator() {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Treatments / Month</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatNumber(result.breakEvenTreatments)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Treatments / Day</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-accent">{formatNumber(result.treatmentsPerDay, 1)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Contribution / Treatment</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.contributionPerTreatment)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      <MedicalSpaCalculatorFooter calculatorId="break-even" relatedTools={relatedTools} dataLinks={medicalSpaDataLinks} faqs={faqs} />
    </>
  );
}
