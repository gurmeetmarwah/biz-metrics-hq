...
import {
  ...
  presets,
  relatedTools,
} from "@/lib/industries/physical-therapy-clinic-startup-cost-calculator";

export function PhysicalTherapyClinicStartupCostCalculator() {
            ...
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                <CurrencyInput
                  ...
                  onChange={(v) => update("leaseholdImprovements", v)}
                />
                <CurrencyInput
                  id="equipment"
                  label="Equipment"
                ...
                <CurrencyInput
                  ...
                  onChange={(v) => update("permitsLicenses", v)}
                />
                <CurrencyInput
                  id="marketing"
                  label="Marketing & Launch"
                  value={inputs.marketingLaunch}
                  onChange={(v) => update("marketingLaunch", v)}
                />
                <CurrencyInput
                  id="working"
                ...
                <CurrencyInput
                  id="acquisition"
                  label="Acquisition Price (if buying)"
                  value={inputs.acquisitionPrice}
                  onChange={(v) => update("acquisitionPrice", v)}
                  tooltip="Leave at $0 for de novo startup"
                />
              </div>
            </div>
            <div className="flex-1">
              <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8 text-center">
                ...
                <p className="mt-3 text-sm text-ink-muted">
                  {result.isAcquisition ? "Acquisition + transition costs" : "De novo startup"} ·{" "}
                  {result.tier}
                </p>
              </div>
