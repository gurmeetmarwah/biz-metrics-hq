...
import { GymCalculatorFooter } from "@/components/calculator/GymCalculatorFooter";
...
import {
  ...
  ltvBenchmarks,
  relatedTools,
} from "@/lib/industries/gym-member-lifetime-value-calculator";

export function GymMemberLifetimeValueCalculator() {
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                ...
                <p className="mt-2 text-sm text-ink-faint">
                  {result.tier} vs benchmark · {formatCurrency(result.vsBenchmark)} vs ~{formatCurrency(gymIndustryAverages.memberLtv)} median
                </p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Avg. Membership Lifespan</p>
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">$500 – $900 typical LTV · 3:1+ LTV:CAC target</p>
                <p className="mt-1 text-xs text-ink-faint">HFA ~{formatCurrency(gymIndustryAverages.revenuePerMemberMonthly)}/mo revenue per member</p>
              </div>
            </div>
          </div>
        </div>
