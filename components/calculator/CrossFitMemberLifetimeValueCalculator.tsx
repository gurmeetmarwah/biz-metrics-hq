...
import { CrossFitCalculatorFooter } from "@/components/calculator/CrossFitCalculatorFooter";
...
import {
  ...
  relatedTools,
} from "@/lib/industries/crossfit-member-lifetime-value-calculator";

export function CrossFitMemberLifetimeValueCalculator() {
  const [inputs, setInputs] = useState(defaultInputs);
  const result = useMemo(() => calculateMemberLtv(inputs), [inputs]);
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              <h2 className="font-display text-lg font-semibold text-ink">Member Economics</h2>
              <p className="mt-1 text-sm text-ink-muted">Model LTV from revenue per member and churn.</p>
              <div className="mt-6 space-y-4">
                <CurrencyInput
                  id="rpc"
                  label="Monthly Revenue Per Member"
                  value={inputs.monthlyRevenuePerMember}
                  onChange={(v) => update("monthlyRevenuePerMember", v)}
                />
                <PercentInput
                  id="churn"
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2 text-center">
                <p className="text-sm text-ink-muted">Member Lifetime Value</p>
                <p className="mt-1 text-4xl font-bold tabular-nums text-metric-valuation">
                  {formatCurrency(result.lifetimeValue)}
                </p>
                <p className="mt-2 text-sm text-ink-faint">
                  {result.tier} vs benchmark · {formatCurrency(result.vsBenchmark)} vs ~{formatCurrency(crossfitIndustryAverages.memberLtv)} median
                </p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Avg. Member Lifespan</p>
              ...
              <div className="rounded-2xl border border-accent/25 bg-accent-soft/30 p-5 sm:col-span-2">
                ...
                <p className="mt-1 font-semibold text-ink">$2,400 – $4,200 typical LTV · 3:1+ LTV:CAC target</p>
                <p className="mt-1 text-xs text-ink-faint">~{formatCurrency(crossfitIndustryAverages.revenuePerMemberMonthly)}/mo revenue per member</p>
              </div>
            </div>
          </div>
        </div>
