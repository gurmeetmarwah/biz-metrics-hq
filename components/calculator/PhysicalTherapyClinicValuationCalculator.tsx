...
import {
  ...
  multipleRanges,
  relatedTools,
} from "@/lib/industries/physical-therapy-clinic-valuation-calculator";

export function PhysicalTherapyClinicValuationCalculator() {
  ...
  const update = <K extends keyof typeof inputs>(key: K, value: (typeof inputs)[K]) => {
    setInputs((prev) => {
      ...
      else if (key === "netProfit" || key === "ownerComp") {
        setSdeManual(false);
        next.sde = estimateSde(next.netProfit, next.ownerComp);
      }
      return next;
    });
  };
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              <h2 className="font-display text-lg font-semibold text-ink">Clinic Financials</h2>
              <div className="mt-6 space-y-4">
                ...
                <CurrencyInput id="ownerComp" label="Owner Physical Therapist Compensation" value={inputs.ownerComp} onChange={(v) => update("ownerComp", v)} />
                <CurrencyInput id="sde" label="SDE" value={inputs.sde} onChange={(v) => update("sde", v)} tooltip={sdeManual ? "Manual override" : "Auto: Net Profit + Owner Compensation"} />
                <CurrencyInput id="equipment" label="Equipment Asset Value" value={inputs.equipmentValue} onChange={(v) => update("equipmentValue", v)} />
                <label htmlFor="multiple" className="block">
                  <span className="text-sm font-medium text-ink-muted">SDE Multiple</span>
                  <input id="multiple" type="number" min={1} max={8} step={0.05} value={inputs.sdeMultiple}
                    onChange={(e) => update("sdeMultiple", Number(e.target.value) || 3.2)}
          ...
          <ul className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {multipleRanges.map((row) => (
              <li key={row.label} className="rounded-2xl border border-border bg-surface p-5 text-center">
                ...
                {"median" in row && typeof row.median === "number" && row.median > 0 && (
      ...
      <PhysicalTherapyCalculatorFooter calculatorId="valuation" relatedTools={relatedTools} dataLinks={physicalTherapyDataLinks} faqs={faqs} />
    </>
  );
}
