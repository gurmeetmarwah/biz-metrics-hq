...
import {
  ...
  relatedTools,
} from "@/lib/industries/pool-service-profit-calculator";

export function PoolServiceProfitCalculator() {
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                <CurrencyInput id="payroll" label="Technician Payroll" value={inputs.technicianPayroll} onChange={(v) => update("technicianPayroll", v)} />
                <CurrencyInput id="chemicals" label="Chemicals & Supplies" value={inputs.chemicals} onChange={(v) => update("chemicals", v)} />
                <CurrencyInput id="fleet" label="Fleet Costs" value={inputs.fleet} onChange={(v) => update("fleet", v)} />
                <CurrencyInput id="marketing" label="Marketing" value={inputs.marketing} onChange={(v) => update("marketing", v)} />
                <CurrencyInput id="overhead" label="Overhead & Admin" value={inputs.overhead} onChange={(v) => update("overhead", v)} />
              </div>
            </div>
            <div className="flex-1">
              <div className="rounded-2xl border border-border bg-surface-muted/30 p-6 sm:p-8">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    <p className="text-sm text-ink-muted">Net Margin</p>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatPercent(result.grossMarginPct)}</p>
                  </div>
                </div>
                <div className="mt-6 rounded-xl border border-border bg-surface px-5 py-4">
                  ...
                  <p className="mt-1 font-semibold text-ink">Average Pool Service: {poolServiceIndustryAverages.marginRange}</p>
                  <p className="mt-1 text-xs text-ink-faint">Median {poolServiceIndustryAverages.marginPct}% · 190+ U.S. pool service businesses</p>
                </div>
                <p className="mt-4 inline-flex rounded-full border border-accent/25 bg-accent-soft px-4 py-1.5 text-sm font-semibold text-accent">{result.status}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <ul className="mt-8 space-y-3 rounded-2xl border border-border bg-surface p-6">
            {result.breakdown.map((item) => (
              <li key={item.label} className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold tabular-nums text-ink">{formatCurrency(item.amount)} ({formatPercent(item.pct, 0)})</span>
              </li>
            ))}
          </ul>
