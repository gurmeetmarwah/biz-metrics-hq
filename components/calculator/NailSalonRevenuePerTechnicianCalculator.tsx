...
import {
  ...
  nailSalonDataLinks,
  relatedTools,
} from "@/lib/industries/nail-salon-revenue-per-technician-calculator";

export function NailSalonRevenuePerTechnicianCalculator() {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
            <div className="flex-1 lg:max-w-md">
              ...
              <div className="mt-6 space-y-4">
                ...
                <label htmlFor="techs" className="block">
                  <span className="text-sm font-medium text-ink-muted">Number of Technicians</span>
                  <input id="techs" type="number" min={1} value={inputs.numberOfTechnicians}
                    onChange={(e) => update("numberOfTechnicians", Number(e.target.value) || 1)}
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-border bg-surface p-5 sm:col-span-2">
                ...
                <p className="mt-1 text-4xl font-bold tabular-nums text-metric-revenue">{formatCurrency(result.perTechnician)}</p>
                <p className="mt-2 text-sm capitalize text-ink-muted">Status: {result.status}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Monthly per Technician</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-ink">{formatCurrency(result.monthlyPerTechnician)}</p>
              </div>
              <div className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs text-ink-faint">Weekly per Technician</p>
                <p className="mt-1 text-2xl font-bold tabular-nums text-accent">{formatCurrency(result.weeklyPerTechnician)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <NailSalonCalculatorFooter calculatorId="revenue-per-technician" relatedTools={relatedTools} dataLinks={nailSalonDataLinks} faqs={faqs} />
    </>
  );
}
