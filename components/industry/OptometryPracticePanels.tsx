...
export function PatientMetricsPanel({
  title,
  subtitle,
  items,
}: {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border-2 border-metric-revenue/25 bg-gradient-to-br from-surface via-surface to-metric-revenue/5 shadow-sm">
      ...
      <dl className="grid grid-cols-1 gap-px bg-border-subtle sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <div key={item.metric} className="bg-surface px-6 py-5 sm:px-8">
            <dt className="text-sm text-ink-muted">{item.metric}</dt>
            <dd className="mt-2 text-xl font-bold tabular-nums text-metric-revenue">{item.value}</dd>
          </div>
        ))}
      </dl>
    </div>
...
export function RetailSalesDashboard({
  title,
  subtitle,
  items,
}: {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border-2 border-accent/25 bg-gradient-to-br from-surface via-surface to-accent-soft/20 shadow-sm">
      <div className="border-b border-border-subtle bg-surface-muted/40 px-6 py-5 sm:px-8">
...
export function InsuranceMetricsPanel({
  title,
  subtitle,
  items,
}: {
...
type PracticeEconomicsComparison = {
...
export function PracticeEconomicsMatrix({
  title,
  subtitle,
  data,
}: {
  title: string;
  subtitle?: string;
  data: PracticeEconomicsComparison;
}) {
  return (
