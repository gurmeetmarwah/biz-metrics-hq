...
type Thresholds = {
  ...
  operatories: { low: number; median: number; high: number };
};
...
export function DentalBenchmarkTool({ thresholds }: { thresholds: Thresholds }) {
  const [revenue, setRevenue] = useState("1800000");
  const [margin, setMargin] = useState("24");
  const [patients, setPatients] = useState("1800");
  const [operatories, setOperatories] = useState("5");

  const rev = parseFloat(revenue) || 0;
  const mar = parseFloat(margin) || 0;
  const pts = parseFloat(patients) || 0;
  const ops = parseFloat(operatories) || 0;

  const results = [
    { label: "Revenue", rating: rate(rev, thresholds.revenue) },
    { label: "Profit Margin", rating: rate(mar, thresholds.ebitdaMargin) },
    { label: "Patient Count", rating: rate(pts, thresholds.patientCount) },
    { label: "Operatories", rating: rate(ops, thresholds.operatories) },
  ];
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
      <h3 className="font-display text-lg font-semibold text-ink">Benchmark Your Practice</h3>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className="block">
          <span className="text-sm font-medium text-ink-muted">Active Patient Count</span>
          <input
            type="number"
            value={patients}
        ...
        <label className="block">
          <span className="text-sm font-medium text-ink-muted">Operatories</span>
          <input
            type="number"
            value={operatories}
            onChange={(e) => setOperatories(e.target.value)}
            className="mt-1.5 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm tabular-nums text-ink focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10"
          />
        </label>
      </div>
